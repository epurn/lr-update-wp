...
local function get_logger()
  if not logger then
    logger = LrLogger('site_publisher')
...
local function write_documents_file(message)
  -- Keep in place, harmless even if folder not present
  ...
  local log_file = LrPathUtils.child(logs_dir, 'site_publisher.log')
  LrFileUtils.createAllDirectories(logs_dir)
  local fh = io.open(log_file, 'a')
  if fh then
    fh:write(os.date('!%Y-%m-%dT%H:%M:%SZ'), ' ', message, '\n')
...
function M.get_paths()
  ...
  if plugin_dir then
    ...
    plugin_file = LrPathUtils.child(plugin_logs, 'site-publisher.log')
  end
  return {
...
function M.test_write()
  ...
  local plugin_ok, docs_ok = false, false
